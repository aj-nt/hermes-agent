...
SKILL_MANAGE_SCHEMA = {
    ...
    "description": (
        ...
        "After difficult/iterative tasks, consider whether the session produced reusable "
        "procedural knowledge — non-obvious workflows, trial-and-error discoveries, "
        "or pitfalls that would waste time next session. If so, offer to save as a skill. "
        "Skip offering when the task's artifact is code and the procedure was "
        "straightforward (install, configure, subclass). Most hard tasks don't need "
        "skills — the code is the record. Confirm with user before creating/deleting.\n\n"
